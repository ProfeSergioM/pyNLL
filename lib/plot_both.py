...
def plot_both(voldf,hypo,NLL,evsel,strid,year,idev,angle):
    ...
    mapa1.set_title('Localización mediante Hypo71,NLL y cálculo de FOCMEC',fontsize=10)
    ...
    ax = plt.axes([0, 0, 1, 1], projection='stereonet')
    inset_x = 0.87
    inset_y = 0.7
    inset_size = 0.2
    ip = InsetPosition(mapa1, [inset_x - inset_size / 2,
    ...
    if (desv_dip > angle) or (desv_rake > angle):
        print('Desviación estándar de soluciones FOCMEC mayor a '+str(angle))
        return None
    ...
    filename=voldf.nombre_db.iloc[0]+'_'+str(evsel.fecha.iloc[0])[0:19].replace(' ','T').replace(':','-')+'_'+evsel.tipoevento.iloc[0]
    plt.savefig('saved_locs/'+filename+'_both.png',dpi=200)
    plt.close('all')
    import shutil,os
    try:
        os.remove('gmt.conf')
        ...
        os.remove('FOCMEC.inp')
    except:
        ()
    shutil.copyfile('saved_locs/'+filename+'_both.png', '/mnt/puntocientodos/tmp/pyNLL/'+filename+'_both.png')
